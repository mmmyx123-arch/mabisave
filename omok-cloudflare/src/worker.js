...
export default {async fetch(request,env){
 ...
 if(u.pathname==='/health')return Response.json({ok:true,version:'2.6.0'});
 if(u.pathname.startsWith('/room/')){
  const code=u.pathname.slice(6).toUpperCase();if(!/^[A-Z0-9]{6}$/.test(code))return new Response('Invalid room',{status:400});
