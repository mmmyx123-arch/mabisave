...
export class GameRoom {
 ...
 async fetch(request){
  ...
  const matchmaking=path==='/match',lobby=path==='/lobby';
 ...
 async webSocketClose(ws){if(ws.deserializeAttachment()?.kind==='lobby'){ws.serializeAttachment({kind:'lobby',closed:true});setTimeout(()=>this.broadcastLobby(this.ctx.storage.get('rooms').then?{}:{}),0);return;}if(ws.deserializeAttachment()?.kind==='match'){ws.serializeAttachment({kind:'match',queued:false});return;}ws.serializeAttachment({id:null});this.broadcast();}
